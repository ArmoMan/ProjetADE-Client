...
class CapteurArduinoWaterSensor(CapteurAnalogiqueParent):
    ...
    def mesurer_donnees(self):
        ...
        if voltage > self.__voltage_max :
            pourcentage_immersion = 100.0
        elif voltage < self.__voltage_min:
            pourcentage_immersion = 0.0
        else:
            pourcentage_immersion = ((voltage - self.__voltage_min) / (self.__voltage_max  - self.__voltage_min)) * 100
